...
# Filter training data to only potentially winning targets (score <= 4)
low_score_mask = y_train <= 4
X_train = X_train[low_score_mask]
y_train = y_train[low_score_mask]

print(f"Filtered training data to custom_score <= 4")
print(f"Training samples: {len(y_train)} (from {len(pd.concat(train)['custom_score'])})")

# Select specific columns
feature_cols = [
    "Specificity_Combo",
    "N_Off_Targets_1.0", "N_Off_Targets_0.5", "SC_2nd_Target_Val", 
    "On_Val_50", "On_Val_75",
...
# Scale features by their weights
...
print(f"Feature weights applied: {feature_weights}")
...
pd.concat([
    df_ind2.loc[df_ind2["gene_name"]=="NECTIN4_PODXL2"].iloc[0],
    df_ind2.loc[df_ind2["gene_name"]=="NECTIN4_B4GALNT4"].iloc[0]
], axis=1)
