...
# ============================================================================
# EVALUATE STANDARD REGRESSION MODEL
# ============================================================================
...
print(f"\nStandard Regression Test Set Performance:")
...
# ============================================================================
# QUANTILE REGRESSION - Focus on low scores
# ============================================================================
print("\n" + "="*80)
print("QUANTILE REGRESSION (Focus on low-scoring targets)")
print("="*80)
...
print("Training quantile regression model (10th percentile)...")
quantile_model.fit(X_train_scaled, y_train)

# Evaluate quantile model
y_pred_quantile = quantile_model.predict(X_test_scaled)

correlation_quantile = np.corrcoef(y_test, y_pred_quantile)[0, 1]
rmse_quantile = np.sqrt(np.mean((y_test - y_pred_quantile) ** 2))
r2_quantile = 1 - (np.sum((y_test - y_pred_quantile) ** 2) / np.sum((y_test - y_test.mean()) ** 2))

print(f"\nQuantile Regression (α=0.1) Test Performance:")
print(f"  Correlation: {correlation_quantile:.4f}")
print(f"  RMSE: {rmse_quantile:.4f}")
print(f"  R² Score: {r2_quantile:.4f}")

# Check bottom 10 targets
actual_bottom_indices = np.argsort(y_test.values)[:10]
pred_bottom_indices_quantile = np.argsort(y_pred_quantile)[:10]
overlap_quantile = len(set(actual_bottom_indices) & set(pred_bottom_indices_quantile))
print(f"  Overlap in bottom 10: {overlap_quantile}/10")

# Feature importance for quantile model
importance_quantile = pd.DataFrame({
    'feature': X_train.columns,
    'importance': quantile_model.feature_importances_
}).sort_values('importance', ascending=False)

print(f"\nQuantile Regression Feature Importance:")
print(importance_quantile)
...
print("="*80)
print(f"Standard Regression  - Bottom 10 Overlap: {overlap}/10")
print(f"Quantile Regression  - Bottom 10 Overlap: {overlap_quantile}/10")
...
I_scaled = scaler.transform(I)
df_ind["predicted"] = best_model.predict(I_scaled)  # All data scaled
df_ind2 = df_ind.loc[df_ind["Positive_Final_v2"] > 50].copy()
df_ind2 = df_ind2[~df_ind2["gene_name"].str.contains("PRAME|LY6K|VTCN1|PCDHB2|CALML5|HIST1H1A|ROS1|SFTPC")]
print(ind)
print(df_ind2[["gene_name", "predicted", "TargetQ_Final_v1", "Positive_Final_v2"]].sort_values("predicted")[0:50])

tabs = set(tid.utils.tabs_genes())
df_ind2 = df_ind2[df_ind2["gene_name"].str.split("_").apply(lambda x: any(g in tabs for g in x))].copy()
df_ind2["combo"] = (100 * (1 - np.maximum(np.minimum(df_ind2["predicted"],7),1) / 7)) + df_ind2["TargetQ_Final_v1"]
print(df_ind2[["gene_name", "combo", "predicted", "TargetQ_Final_v1", "Positive_Final_v2"]].sort_values("combo", ascending = False)[0:50])

df_ind.loc[df_ind["gene_name"]=="TMPRSS4_SLC22A31"].iloc[0]
df_ind.loc[df_ind["gene_name"]=="ABCC3_SLC22A31"].iloc[0]
