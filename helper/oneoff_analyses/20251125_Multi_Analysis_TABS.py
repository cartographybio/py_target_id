...
multi3 = multi2[multi2["claude_summary"] >= 1].copy()
...
multi3["TargetQ_Diff_xy"] = np.maximum(multi3["TargetQ_Final_v2"] - np.maximum(single0["TargetQ_Final_v2"], single1["TargetQ_Final_v2"]), 0)
multi3["TI_Diff_xy"] = np.maximum(multi3["TI"] - np.maximum(single0["TI"], single1["TI"]), 0)
...
multi4[multi4["gene_name"].str.contains("FOLR1")].head()["gene_name"]

top = multi4[(multi4["TABS"] >= 1) & (multi4["Positive_Final_v2"] > 40) & (multi4["TargetQ_Final_v2"] > 85)].sort_values("TargetQ_Final_v2", ascending=False).copy()
...
multi3[multi3["gene_name"]=="NECTIN4_TMEM132A"].iloc[0]
