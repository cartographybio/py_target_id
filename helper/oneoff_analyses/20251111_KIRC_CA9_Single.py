...
#Read Previous
single = pd.concat([
	pd.read_parquet(IND + '.Single.Results.20251111.parquet')
])
...
ref_med_adata[:, ["CA9", "MUCL3"]].X

import numpy as np
from scipy.stats import pearsonr

# Extract the expression matrices for both genes
ca9_expr = ref_med_adata[:, "CA9"].X.toarray().flatten()
mucl3_expr = ref_med_adata[:, "MUCL3"].X.toarray().flatten()

# Calculate Pearson correlation
corr, pval = pearsonr(np.log2(ca9_expr + 1), np.log2(mucl3_expr + 1))

print(f"Pearson correlation: {corr:.4f}")
print(f"P-value: {pval:.4e}")

sub_surf = ref_med_adata[:, ref_med_adata.var_names.isin(surface)]
