...
#Load
fa <- readAAStringSet("Macaca_fascicularis.Macaca_fascicularis_6.0.pep.all.fa")

#3line
line3 <- list.files("../results", pattern = "cyno", full.names = TRUE)
...
extract_str <- function(pattern, str){
    split <- stringr::str_split(str, pattern = pattern, simplify=TRUE)
    stringr::str_split(split[,2], pattern=" ", simplify=TRUE)[,1]
}
...
df <- DataFrame(
    transcript_id = extract_str("transcript:", names(fa)),
    gene_id = extract_str("gene:", names(fa)),
    gene_name = extract_str("gene_symbol:", names(fa)),
    type = split[,2],
    AA = paste0(fa),
    Predict = dt_m$V3
)
df$gene_name[df$gene_name==""] <- df$gene_id[df$gene_name==""]
...
saveRDS(df, "Transcript_Topology_Summary.Cyno_v6.0.rds")
