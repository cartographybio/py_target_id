...
__all__ = ['target_id_multi_v1', 'target_id_multi_v1_safe']
...
def target_id_multi_v1(
    ...
    """
    GPU-optimized target ID pipeline with memory-safe batching.
    ...
    if type(ref_subset).__name__ in ['VirtualAnnData', 'TransposedAnnData']:
    ...
    try:
        ...
        for batch_idx in range(n_batches):
            ...
            malig_med = compute_group_median_cached(
                torch.minimum(malig_X[gx_t], malig_X[gy_t]), m_group_indices
            )
            ref_med = compute_group_median_cached(
                torch.minimum(ref_X[gx_t], ref_X[gy_t]), ref_group_indices
            )
            ...
            results = run.expression_percentiles_by_positivity_multi_gpu_fast(
                malig_med=malig_med,
                gx_indices=gx_t, 
                gy_indices=gy_t,
            ...
            print(f"IterTotal:{iter_total:.1f}s | Total:{total_total/60:.1f}m")

        # -----------------------------
        # Combine results from temp files
        # -----------------------------
        print(f"------  Combining Results - | Total: {time.time()-overall_start:.1f}s")
        df_all = pd.concat([pd.read_parquet(f) for f in tmp_files], ignore_index=True)
